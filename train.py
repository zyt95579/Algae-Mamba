...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):  
            ...
            lovasz_loss = L.lovasz_softmax(outputs_soft, label_batch.unsqueeze(1), ignore=255)
            # loss_dice = dice_loss(outputs_soft, label_batch.unsqueeze(1))
            loss = 0.5 * (loss_ce + lovasz_loss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            writer.add_scalar('info/lovasz_loss', lovasz_loss, iter_num)
            ...
            logging.info(
                'iteration %d : loss : %f, lovasz_loss: %f, loss_ce: %f' %
                (iter_num, loss.item(), lovasz_loss.item(), loss_ce.item()))
            ...
            if iter_num > 0 and iter_num % 300 == 0:
                ...
                image_ids = open('./data/val.txt').read().splitlines()
