...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.act = nn.SiLU()
        self.KAGN = KAGNConv2DLayer(input_dim=self.d_model, output_dim=self.d_model * 2, kernel_size=3, degree=3,
                                    groups=3, padding=1, stride=1, dilation=1, dropout=0.0, norm_layer=nn.InstanceNorm2d)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        B, H, W, C = x.shape
        k = self.KAGN(x.permute(0, 3, 1, 2).contiguous())
        xz = self.in_proj(x)
