...
class KAGNConvNDLayer(nn.Module):
    def __init__(self, conv_class, norm_class, conv_w_fun, input_dim, output_dim, degree, kernel_size,
        ...
        self.attention_conv = nn.Conv2d(groups, groups, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        for group_ind, _x in enumerate(split_x):
            y = self.forward_kag(_x.clone(), group_ind)
            output.append(y.clone())
            attention = torch.mean(y, dim=(1, 2, 3), keepdim=True)

            attentions.append(attention)
        attentions = torch.cat(attentions, dim=1)
        attentions = self.attention_conv(attentions)
        attentions = self.attention_sigmoid(attentions)
        attended_output = []
        for group_ind, y in enumerate(output):
            attended_output.append(y * attentions[:, group_ind:group_ind + 1, :, :])
        y = torch.cat(attended_output, dim=1)
        return y
